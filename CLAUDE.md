...
- **Commit after every change.** When you modify any file in this repo (subscribers, README, CLAUDE.md, manual digest edits, etc.), commit the change with an appropriate message before ending the turn. Use a short imperative subject for human-driven edits (e.g. `update subscribers`, `fix daytona region typo`); reserve the `Run YYYY-MM-DD HH:MM UTC: ...` format for routine-style runs only. Do not batch unrelated edits into one commit.
